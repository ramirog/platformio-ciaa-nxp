...
base_addr = env.BoardConfig().get("upload.base_addr", "")

if upload_protocol == "openocd":
    env.Replace(
        UPLOADER="openocd",
        UPLOADERFLAGS=[
            "-f", env.BoardConfig().get("debug.openocdcfg", ""),
            "-c", "init",
            "-c", "halt 0",
            "-c", "flash write_image erase unlock " + join("$BUILD_DIR", "firmware.bin") + " " + base_addr + " bin",
            "-c", "reset run",
            "-c", "shutdown",
            "-s", join(dirname(platform.manifest_path), "etc", "openocd")
        ],
